...
- Reading the JSX is faster when you don't have to skip past 30 lines of
  style tables to find the markup.
- Constants are the unit of design tweaks. Designers (and you on a Friday)
  will edit `.const.ts` far more often than the JSX. Keep that diff narrow.
- It mirrors `src/components/effects/cinema-scroll/cinema-scroll.const.ts`
  (the project's reference pattern) — and lets greps for "where is this
  color used?" hit a deterministic file name.
- It makes constants importable from sibling sub-components without dragging
  the JSX file onto the import graph.
...
- Color tokens used by inline `style` / CSS vars (not Tailwind classes —
  those stay in JSX)
...
## Where This Pattern Is Already Followed

- `src/components/effects/cinema-scroll/cinema-scroll.const.ts`
- `src/components/layout/footer/const.ts`
- `src/components/ui/preloader/const.ts` (for the preloader-shown key)

## Where It Is *Not* Yet Followed (canonical violations)

- `src/components/ui/text/index.tsx:22–39` (variant table)
- `src/components/ui/skeleton/index.tsx:7` (`SKELETON_BASE_CLASSNAME`)
- `src/components/ui/preloader/index.tsx:14–28` (`CLIP_PATH` and `TIMING`)

These are the canonical "before" examples — fix them and use the diff as a
reference.
