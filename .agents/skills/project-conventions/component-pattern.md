...
in this codebase. Mirrors `src/components/effects/cinema-scroll/` exactly.
Read those files first if you've never seen the pattern in action — they are
the live spec.

## When to Apply

- Creating a new multi-file component under `src/components/`.
- Refactoring a single-file component that has grown subcomponents, internal
  state, or its own constants/types.
- Reviewing PRs that introduce or modify components in scope.

**Do NOT apply** to:
- Trivial primitives (`Container`, `Grid`, `Button`) — keep them as a single
  file.
- `src/components/from-template/` — vendored, leave it alone.
- `src/components/layout/footer/` and `src/components/toast/welcome-toast/` —
  these intentionally use a server/client split pattern that wins over this
  one.
...
rg 'gsap\.(set|to|from|fromTo)\("\[' <dir>  # global selectors — see gsap-selectors.md
